...
- It registers a custom Maplibre protocol via `setupMaplibre` (the protocol is
  `dem-contour://`, derived from the default `id: 'dem'`), then a vector
...
- **Known limitation:** contour intervals are only defined for z12–14, so
  contours appear only when zoomed into that range. Because the map fits a fixed
  15 km box, its default zoom is ~z9–10 on every viewport, meaning contours are
  not visible until you manually zoom in past z12 (and stop again above z14).
  Extending `thresholds` to the lower zooms would show them at the default view.
