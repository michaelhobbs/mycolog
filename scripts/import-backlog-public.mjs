...
function exifDate(file) {
  const out = execFileSync(
    'mdls',
    ['-raw', '-name', 'kMDItemContentCreationDate', file],
    { encoding: 'utf8' },
  );
...
const existingIds = (await fs.readdir(BACKLOG))
  .filter((n) => /^\d+$/.test(n))
  .map(Number);
...
for (const folder of folders) {
  ...
  files.forEach((f, i) => {
    ...
    execFileSync('sips', ['-s', 'format', 'jpeg', path.join(srcDir, f), '--out', out], { stdio: 'ignore' });
